...
export default function CreateIssueForm() {
	...
	const onSubmit = async (formValues: TIssueCreateSchema) => {
		if (!apartment?.id) {
			toast.error(
				"Create your apartment first in your profile, before creating and issue",
			);
			return;
		}
		...
		try {
			...
			toast.success(
				"Your issue has been reported. A confirmation email has been sent to your email.",
			);
			reset();
			router.push("/profile");
		} catch (error) {
			...
			>
				...
				<div>
					...
					<div>
						...
						<div className="m-2 flex items-center space-x-3 text-sm">
							<ClientOnly>
								<Controller
									name="priority"
									control={control}
									render={({ field: { onChange, onBlur, value } }) => (
										<Select
											...
											onChange={(val) => onChange(val?.value)}
											onBlur={onBlur}
											placeholder="Set your issue priority"
