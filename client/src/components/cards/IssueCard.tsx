...
export default function IssueCard({ issue }: IssueCardProps) {
	return (
		<Link href={`/issue/${issue.id}`} key={issue.id}>
			<Card
				key={issue.id}
				className="hover:border-pumpkin dark:border-gray hover:dark:border-platinum rounded-xl border border-dashed cursor-pointer"
			>
				<CardHeader className="flex-center h3-semibold font-robotoSlab dark:text-lime-500">
					<CardTitle>
				...
				<CardFooter className="dark:text-babyPowder flex flex-row justify-between">
					<p>
						...
						<Badge className="bg-eerieBlack text-babyPowder dark:bg-electricIndigo dark:text-babyPowder rounded-xl">
							{issue.status}
						</Badge>
					</p>
					...
					<p>
						...
						<Badge className="bg-eerieBlack text-babyPowder dark:text-veryBlack dark:bg-lime-500 rounded-xl">
							{issue.priority}
						</Badge>
					</p>
				</CardFooter>
