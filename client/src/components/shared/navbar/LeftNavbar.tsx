    ...
    <section className='bg-baby_rich light-border custom-scrollbar shadow-platinum sticky left-0 top-0 flex h-screen flex-col justify-between overflow-y-auto border-r p-6 pt-36 max-sm:hidden lg:w-[297px] dark:shadow-none'>
      <div className='flex flex-1 flex-col gap-6'>
        {leftNavLinks.map((linkItem) => {
          ...
          return (
            <Link href={linkItem.path} key={linkItem.label} className={`${isActive ? "electricIndigo-gradient rounded-lg text-babyPowder" : "text-baby_richBlack"} flex items-center justify-start gap-4 bg-transparent p-4`}>
              <Image 
      ...
      <div className='flex flex-col gap-3'>
        ...
        <Link href="/register">
          <Button className="pumpkin-gradient small-medium light-border-2 btn-tertiary text-baby_ballon min-h-[41px] w-full rounded-lg border px-4 py-3 shadow-none">
            Register
          </Button>
        </Link>
      </div>
